...
window.addEventListener("load", () => {
  ...
  document
    .getElementById("sort-by-dislikes")
    .addEventListener("click", (event) => {
      const articles = Array.from(document.querySelectorAll("#articles ul li"));
...
function redrawCurrentPage() {
  ...
  const maxPage = Math.ceil(
    document.querySelectorAll("#articles ul li").length / PAGE_SIZE
  );
  ...
  for (let index = 0; index < articles.length; index++) {
    if (
      PAGE_SIZE * (current_page - 1) <= index &&
...
async function deleteArticle(event) {
  try {
    ...
    const deleteUrl = window.location.origin + "/article/" + articleId;
